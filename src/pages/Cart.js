...
const Cart = () => {
  const { cartItems, setCartItems, onAdd, onRemove, priceSum, itemsPrice, removeCompletely, shippingPrice } = useCart();
      ...
      <div className="cart-container px-5 py-10 w-[75%] mx-auto flex items-center">
        <div className="cart-left">
          <h4 className="car-header text-3xl py-3 my-5  ">
            Basket ({cartItems.length} products)
          </h4>
                ...
                <>
                  <div className="single-product flex  items-center justify-between border w-[50vw] ">
                    ...
                    <div className="product-desc flex grow justify-around  mx-5  ">
                      ...
                      <div>
                          <button onClick={()=> onRemove(item)} className= "px-[4px] rounded bg-red-500">-</button>
                          <span className="px-[7px]">
                          {item.qty}
                          </span>
    
                          <button onClick={()=>{onAdd(item)}} className= "px-[4px] bg-blue-500 rounded">+</button></div>
        ...
        <div className="cart-right whitespace-nowrap mx-5 border p-[3rem] grow ">
          ...
          <ul className="flex justify-between text-[15px] py-5">
            <li className="flex flex-col font-semibold">
              <span>Subtotal</span>
             {  itemsPrice > 20 && itemsPrice < 100 ?  <span className="text-xs">{  `Add ${ (   itemsPrice < 100   && (100 - itemsPrice).toFixed(2) )} for free shipping`}</span> : null}
            </li>
          ...
          <ul className="flex justify-between text-[15px] py-5">
            ...
            <li className="font-bold">{(itemsPrice + shippingPrice).toFixed(2)}$</li>
          </ul>
        </div>
      </div>) 
      :
       ...
       <div className=" border w-[40vw] h-[20vh] mx-auto flex flex-col items-center justify-center">
         <p className="py-5 text-2xl font-serif">
            No item in Basket
         </p>
         <div className="py-5">
