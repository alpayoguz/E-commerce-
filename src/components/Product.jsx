...
const Product = ({ item }) => {
  ...
  return (
    <div
      ...
      className="product-container relative w-[400px] h-[250px] flex flex-col justify-center p-3 m-1 bg-[#f5fbfd]   hover:opacity-75"
    >
      <div className="product-img-container h-[60%] w-full">
        <img className="w-full h-[100%] object-contain" src={item.img} />
      </div>
      <div className="flex flex-col justify-around px-3 fle flex-co product-desc">
        <p className="text-[1rem] xl:text-base font-semibold">
          {item.productName}
        </p>
        <p className="text-[1rem] xl:text-base font-bold">${item.price}</p>
      </div>
      <div
